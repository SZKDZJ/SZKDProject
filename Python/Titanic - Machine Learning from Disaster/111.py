...
for pclass in range(1, 4):
    for sex in ['female', 'male']:
        print('Median age of Pclass {} {}s: {}'.format(pclass, sex, median_age_by_pclass_sex[
            (df_all['Sex'] == sex) & (df_all['Pclass'] == pclass)].iloc[0]))
...
for column in df_test.select_dtypes(include=['object']).columns:
    ...
    # 检查 column 是否在 df_test 中
    if column in df_test.columns:
        # 确保 X_train 和 df_test 中的列类型一致，转换为字符串
        X_train[column] = X_train[column].astype(str)
        df_test[column] = df_test[column].astype(str)

        # 合并训练集和测试集中的该列数据，用于标签编码
        combined_categories = pd.concat([X_train[column], df_test[column]])
        ...
        print(f"Column {column} is missing in df_test")
