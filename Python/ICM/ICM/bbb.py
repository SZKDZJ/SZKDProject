...
# 使用多线程加速计算，并且加上进度条
def compute_shortest_paths(bridge_nodes):
    results = {}
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_nodes = {
            executor.submit(a_star_shortest_path, network.G, start, end): (start, end)
            for i, start in enumerate(bridge_nodes)
            for end in bridge_nodes[i + 1:]
        }

        for future in tqdm(concurrent.futures.as_completed(future_to_nodes), total=len(future_to_nodes), desc="Computing shortest paths"):
            start, end = future_to_nodes[future]
            try:
                path, time = future.result()
                results[(start, end)] = (path, time)
            except Exception as exc:
                print(f"Error processing {start}-{end}: {exc}")

    return results

# 计算两两最短路径
results = compute_shortest_paths(list(bridge_nodes))
